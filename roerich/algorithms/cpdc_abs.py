...
# Bootstrap 
def bs_split(X_ref, X_test):
    assert len(X_ref) == len(X_test)
    n = len(X_ref)

    inds = np.random.choice(n, size=n, replace=True)
    X_ref_bs = X_ref[inds]
    X_test_bs = X_test[inds]
    y_ref_bs = np.zeros(len(X_ref_bs))
    y_test_bs = np.ones(len(X_test_bs))

    shuffle_inds = np.random.choice(len(X_ref_bs)+len(X_test_bs), size=len(X_ref_bs)+len(X_test_bs), replace=False)
    X_train = np.concatenate([X_ref_bs, X_test_bs])[shuffle_inds]
    y_train = np.concatenate([y_ref_bs, y_test_bs])[shuffle_inds]

    rest_inds = list(set(list(np.arange(n))) - set(inds))
    X_ref_bs = X_ref[rest_inds]
    X_test_bs = X_test[rest_inds]
    y_ref_bs = np.zeros(len(X_ref_bs))
    y_test_bs = np.ones(len(X_test_bs))

    shuffle_inds = np.random.choice(len(X_ref_bs)+len(X_test_bs), size=len(X_ref_bs)+len(X_test_bs), replace=False)
    X_val = np.concatenate([X_ref_bs, X_test_bs])[shuffle_inds]
    y_val = np.concatenate([y_ref_bs, y_test_bs])[shuffle_inds]

    assert sum(y_train)/len(y_train) == sum(y_val)/len(y_val) == 0.5
...
# Classification
class ChangePointDetectionClassifierABS(ChangePointDetectionBase):
    ...
    def reference_test_predict(self, X_ref, X_test):
        ...
        for j in range(self.n_samples):
            X_train, X_val, y_train, y_val = bs_split(X_ref, X_test)

            classifier = deepcopy(self.base_classifier)
            classifier.fit(X_train, y_train)
            y_pred = classifier.predict_proba(X_val)[:, 1]
            
            ref_preds = y_pred[y_val == 0]
            test_preds = y_pred[y_val == 1]
            ratios = self.densratio(y_pred)
            ref_ratios = ratios[y_val == 0]
            test_ratios = ratios[y_val == 1]
            ...
            if self.metric == "KL":
                ...
                score = 2 * (roc_auc_score(y_val, y_pred) - 0.5)
            else:
                score = 0
